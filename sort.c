...
/*   Updated: 2026/04/18 13:50:42 by pkongkha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	sort(struct s_op_info *info, struct s_chunk *chunk)
{
	...
	status = sort(info, &(struct s_chunk){.loc = res.pol.max_to,
			.size = res.move_max});
	if (status)
		return (status);
	status = sort(info, &(struct s_chunk){.loc = res.pol.mid_to,
			.size = res.move_mid});
	if (status)
		return (status);
	status = sort(info, &(struct s_chunk){.loc = res.pol.min_to,
			.size = res.move_min});
	if (status)
		return (status);
	return (0);
